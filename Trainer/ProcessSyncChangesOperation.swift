...
class ProcessSyncChangesOperation: Operation {
    ...
    fileprivate var changedCloudKitManagedObjects: [CloudKitManagedObject]
    ...
    func processServerDeletions(_ managedObjectContext: NSManagedObjectContext) {
        ...
        for deletedServerRecordID in preProcessServerDeletedRecordIDs {
            ...
            if let index = changedCloudKitManagedObjects.index( where: { $0.recordName == deletedServerRecordID.recordName } ) {
    ...
    func processConflicts(_ managedObjectContext: NSManagedObjectContext) {
    ...
    func resolveConflict(_ recordName: String, managedObjectContext: NSManagedObjectContext) {
        ...
        guard let serverChangedRecordIndex = preProcessServerChangedRecords.index( where: { $0.recordID.recordName == recordName } ),
            let localChangedObjectIndex = changedCloudKitManagedObjects.index( where: { $0.recordName == recordName } ) else {
        ...
        if serverChangedRecordLastUpdate.compare(localChangedObjectLastUpdate as Date) == ComparisonResult.orderedDescending {
        ...
        } else if serverChangedRecordLastUpdate.compare(localChangedObjectLastUpdate as Date) == ComparisonResult.orderedAscending {
